...
FastLinkedList<_Tp, _N>::FastLinkedList(const _Tp& default_value)
    : list_{}, p_start_node_(_N), p_end_node_(_N), p_next_available_node_(_N - 1), num_nodes_(0) {
  static_assert(_N != 0);
...
void FastLinkedList<_Tp, _N>::push_front(const _Tp& value) {
  if (!full()) {
    ...
    auto p_new_next_available_node = list_.at(p_next_available_node_).p_next_node;
    ...
    list_.at(p_next_available_node_).value       = value;
    list_.at(p_next_available_node_).p_next_node = p_start_node_;
    p_start_node_                                = p_next_available_node_;

    // the new next_available_node will now be assigned the saved value
    p_next_available_node_ = p_new_next_available_node;
...
void FastLinkedList<_Tp, _N>::push_front(_Tp&& value) {
  if (!full()) {
    ...
    // then update the pointer of the current node to point at the node that p_start_node_ was previously pointing at
    // finally, update the p_start_node_ pointer to point at the new node
    list_.at(p_next_available_node_).value       = std::move(value);
    list_.at(p_next_available_node_).p_next_node = p_start_node_;
    p_start_node_                                = p_next_available_node_;
...
void FastLinkedList<_Tp, _N>::ReturnToAvailableNodesList(size_type p_new_available_node) {
  list_.at(p_new_available_node).p_next_node = p_next_available_node_;
